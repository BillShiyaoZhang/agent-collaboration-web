...
export async function createControlCall(user: User, agent: Agent, call: { request_id: string; method: string; params: Record<string, unknown> }) {
  ...
  if (!row) {
    ...
    try {
      row = await prisma.controlRequest.create({ data: { id: call.request_id, agentId: agent.id, consoleUrn: user.virtualUrn,
        method: call.method, fingerprint, requestEnvelope, deadline, expiresAt: new Date(now + RETENTION_MS) } });
    } catch (error) {
  ...
  return controlCallResult(user, agent, row);
}
...
export async function pollControlResponses(user: User) {
  await cleanupControlCache();
  ...
  for (const item of items) {
    ...
    if (!row.responseEnvelope && row.deadline.getTime() > Date.now()) {
      // Persist authenticated wire bytes before ACK. The cache never becomes a contact/task/message store.
      await prisma.controlRequest.updateMany({ where: { id: row.id, responseEnvelope: null }, data: { responseEnvelope: item.payload_proto, status: "complete" } });
    }
