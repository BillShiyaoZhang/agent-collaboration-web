...
export function AddContactPanel({ workbench: w }: { workbench: Workbench }) {
  ...
  return <section className="mx-3 mb-5 rounded-2xl border bg-muted/20 p-3 sm:mx-5 sm:p-4" aria-label="添加联系人">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-sm font-medium">添加联系人</h3><p className="mt-1 text-xs leading-6 text-muted-foreground">填写对方的称呼与 URN，本机 agent 会将好友请求排队并尝试投递。对方收到并接受后，双方才建立连接。</p></div>
    ...
    {shown && !locked && <form onSubmit={submit} className="mt-4 space-y-4">
      ...
      <div><label htmlFor="contact-urn" className="text-xs font-medium">对方的 URN</label><Input id="contact-urn" autoComplete="off" autoCapitalize="none" spellCheck={false} required maxLength={256} value={urn} onChange={event => { setUrn(event.target.value); setConfirmed(false); }} placeholder="urn:agent-comm:agent:…" className="mt-1.5 font-mono text-base" disabled={!allowed} /><p className="mt-1.5 text-xs leading-6 text-muted-foreground">请向对方获取其 agent 的完整 URN。使用 v2 时，双方还须各自通过独立渠道核对并在本机固定对方完整的 Ed25519 公钥，才能投递好友请求；此网页无法代为固定公钥。</p></div>
      <label className="flex cursor-pointer items-start gap-2.5 rounded-xl border bg-background p-3 text-xs leading-6"><input type="checkbox" checked={confirmed} onChange={event => setConfirmed(event.target.checked)} disabled={!allowed || !name.trim() || !urn.trim()} className="mt-1 h-4 w-4 shrink-0 accent-primary" /><span>我已核对，确认{aliases.length ? `「${aliases.join("、")}」` : "以上称呼"}对应此 URN：<span className="mt-1 block break-all font-mono">{urn.trim() || "填写后将在这里显示"}</span><span className="mt-1 block text-muted-foreground">请求可能仍待投递；对方收到并接受后才建立好友连接。</span></span></label>
