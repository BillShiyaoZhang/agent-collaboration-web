...
export function AddContactPanel({ workbench: w }: { workbench: Workbench }) {
  ...
  return <section className="mx-3 mb-5 rounded-2xl border bg-muted/20 p-3 sm:mx-5 sm:p-4" aria-label="添加联系人">
    ...
    {shown && !locked && <form onSubmit={submit} className="mt-4 space-y-4">
      ...
      <div><label htmlFor="contact-urn" className="text-xs font-medium">对方的 URN</label><Input id="contact-urn" autoComplete="off" autoCapitalize="none" spellCheck={false} required maxLength={256} value={urn} onChange={event => { setUrn(event.target.value); setConfirmed(false); }} placeholder="urn:agent-comm:agent:…" className="mt-1.5 font-mono text-base" disabled={!allowed} /><p className="mt-1.5 text-xs leading-6 text-muted-foreground">请通过可信渠道获取对方 agent 的准确 URN。v0.9.0 helper 可自动核对公钥；旧 v0.8.0 仍要求双方在本机核对并固定完整公钥。请按实际安装版本操作。</p></div>
