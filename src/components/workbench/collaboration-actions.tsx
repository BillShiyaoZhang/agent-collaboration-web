...
import { GoalWorkflow } from "./collaboration-workflow";
...
export function CollaborationActions({ workbench: w, initialOpen = false }: { workbench: Workbench; initialOpen?: boolean }) {
  ...
  const result = w.mutations.actions.slice().sort((a, b) => Number(b.updatedAt || b.createdAt || 0) - Number(a.updatedAt || a.createdAt || 0)).find(item => item.call.method === "collaboration.execute");
  async function load() {
