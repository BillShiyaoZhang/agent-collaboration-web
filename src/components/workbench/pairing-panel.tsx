  ...
  return <section className="overflow-hidden rounded-2xl border bg-card shadow-sm" aria-label="控制台配对">
    ...
    {w.pairingOpen && <div id="pairing-panel" className="border-t p-5 sm:p-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.1fr_1fr]">
        <div><h2 className="font-medium">{w.identity?.virtualUrn ? "这是你的控制台身份" : "创建一个专属控制台身份"}</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">{w.identity?.virtualUrn ? "复制下面的 URN，在 agent 本机授权这个控制台。" : "身份用于让 agent 验证请求来源。创建后，在本机完成一次配对即可开始。"}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">这是网页服务托管的身份；网页服务可以读取 agent 授权发给控制台的内容。</p>
