...
export function PairingPanel({ workbench: w, agent, featureCount }: { workbench: Workbench; agent: Connection; featureCount: number }) {
  ...
  return <section className="overflow-hidden rounded-2xl border bg-card shadow-sm" aria-label="控制台配对">
    <button type="button" className="flex w-full items-center justify-between gap-3 px-5 py-4 text-left outline-none transition-colors hover:bg-muted/35 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring" onClick={() => w.setPairingOpen(previous => !previous)} aria-expanded={w.pairingOpen} aria-controls="pairing-panel">
      <span className="flex items-center gap-3"><span className={cn("rounded-xl p-2", w.capabilitySnapshot ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground")}>{w.capabilitySnapshot ? <ShieldCheck className="h-4 w-4" /> : <KeyRound className="h-4 w-4" />}</span><span><span className="block text-sm font-medium">{w.capabilitySnapshot ? "已收到 agent 的认证响应" : "先让你的 agent 认识这个控制台"}</span><span className="mt-1 block text-xs leading-5 text-muted-foreground">{w.capabilitySnapshot ? `最近检查 ${displayTime(w.capabilitySnapshot.time / 1000)} · ${featureCount} 项功能可用` : "完成本机配对，开启你的远程工作台。"}</span></span></span><ChevronDown className={cn("h-4 w-4 shrink-0 text-muted-foreground transition-transform", w.pairingOpen && "rotate-180")} />
    </button>
    {w.pairingOpen && <div id="pairing-panel" className="border-t p-5 sm:p-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.1fr_1fr]">
        ...
        <div className="rounded-2xl bg-muted/40 p-4 sm:p-5"><h3 className="text-sm font-medium">在 agent 本机完成配对</h3><p className="mt-2 text-sm leading-7 text-muted-foreground">将控制台 URN 加入本机配对记录，并加入 connector 的 <code className="rounded bg-background px-1 py-0.5 text-xs">allow_from</code>。保持 agent 与 helper 运行，然后检查连接。</p><p className="mt-2 text-xs leading-6 text-muted-foreground">保存连接或注册身份后，仍需要本机授权。</p><div className="mt-4 flex flex-wrap gap-2"><Button className="gap-2 rounded-xl" disabled={w.identityBusy || !w.identity?.virtualUrn || !!w.busy.capabilities} onClick={() => void w.invoke("capabilities")}>{w.busy.capabilities ? <Loader2 className="h-4 w-4 animate-spin" /> : <ArrowRight className="h-4 w-4" />}{w.busy.capabilities ? "检查中…" : w.capabilitySnapshot ? "重新检查连接" : "我已配对，检查连接"}</Button>{w.identity?.virtualUrn && <Button variant="ghost" size="sm" className="rounded-xl text-xs" disabled={w.identityBusy || !!w.busy.capabilities} onClick={w.createIdentity}>{w.identityBusy ? "注册中…" : "重新注册身份"}</Button>}</div></div>
      </div>
      ...
      {!!unavailable.length && <details className="mt-3"><summary className="cursor-pointer text-xs text-muted-foreground">未开放的功能</summary><ul className="mt-3 space-y-2 text-xs leading-5 text-muted-foreground">{unavailable.map(method => <li key={string(method.name)}><span className="font-medium text-foreground">{methodLabels[string(method.name)] || string(method.name)}</span> · {method.name === "approval.respond" ? "请在 agent 的原生渠道回应确认请求。" : "当前适配器或本机配对尚未开放。"}{typeof method.reason === "string" && <span className="mt-0.5 block break-words text-[11px] opacity-80">{method.reason}</span>}</li>)}</ul></details>}
    </div>}
