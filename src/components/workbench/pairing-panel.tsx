  ...
  return <section className="overflow-hidden rounded-2xl border bg-card shadow-sm" aria-label="控制台配对">
    ...
    {w.pairingOpen && <div id="pairing-panel" className="border-t p-5 sm:p-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.1fr_1fr]">
        ...
        <div className="rounded-2xl bg-muted/40 p-4 sm:p-5"><h3 className="text-sm font-medium">在 agent 本机完成配对</h3><p className="mt-2 text-sm leading-7 text-muted-foreground">将控制台 URN 加入本机配对记录，并加入 connector 的 <code className="rounded bg-background px-1 py-0.5 text-xs">allow_from</code>。保持 agent 与 helper 运行，工作台会自动检查连接。</p><p className="mt-2 text-xs leading-6 text-muted-foreground">保存连接或注册身份后，仍需要本机授权。</p><div className="mt-4 flex flex-wrap gap-2"><Button className="gap-2 rounded-xl" disabled={w.identityBusy || !w.identity?.virtualUrn || !!w.busy.capabilities} onClick={() => void w.invoke("capabilities")}>{w.busy.capabilities ? <Loader2 className="h-4 w-4 animate-spin" /> : <ArrowRight className="h-4 w-4" />}{w.busy.capabilities ? "检查中…" : w.capabilitySnapshot ? "重新检查连接" : "立即检查连接"}</Button>{w.identity?.virtualUrn && <Button variant="ghost" size="sm" className="rounded-xl text-xs" disabled={w.identityBusy || !!w.busy.capabilities} onClick={w.createIdentity}>{w.identityBusy ? "注册中…" : "重新注册身份"}</Button>}</div></div>
      </div>
