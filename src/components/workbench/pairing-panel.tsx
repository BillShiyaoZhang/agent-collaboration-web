    ...
    {w.pairingOpen && <div id="pairing-panel" className="border-t p-5 sm:p-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.1fr_1fr]">
        <div><h2 className="font-medium">{w.identity?.virtualUrn ? "这是你的控制台身份" : "创建一个专属控制台身份"}</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">{w.identity?.virtualUrn ? "复制下面的 URN，在 agent 本机授权这个控制台。" : "身份用于让 agent 验证请求来源。创建后，在本机完成一次配对即可开始。"}</p>
          {w.identity?.virtualUrn ? <div className="mt-4 rounded-xl border bg-muted/30 p-3"><div className="flex items-center justify-between"><span className="text-xs font-medium text-muted-foreground">控制台 URN</span><CopyValue value={w.identity.virtualUrn} /></div><p className="mt-1 select-all break-all font-mono text-xs leading-6">{w.identity.virtualUrn}</p><details className="mt-3 border-t pt-3"><summary className="cursor-pointer text-xs text-muted-foreground">查看验证公钥</summary><div className="mt-2 flex items-start gap-2"><p className="min-w-0 flex-1 select-all break-all font-mono text-xs leading-5">{w.identity.virtualEd25519PublicKey}</p>{w.identity.virtualEd25519PublicKey && <CopyValue value={w.identity.virtualEd25519PublicKey} label="复制验证公钥" compact />}</div></details></div> : <Button className="mt-4 gap-2 rounded-xl" disabled={w.identityBusy} onClick={w.createIdentity}>{w.identityBusy ? <Loader2 className="h-4 w-4 animate-spin" /> : <KeyRound className="h-4 w-4" />}{w.identityBusy ? "正在读取身份…" : "创建控制台身份"}</Button>}
          {w.identityError && <p role="alert" className="mt-3 text-sm text-destructive">{w.identityError}</p>}
        </div>
        <div className="rounded-2xl bg-muted/40 p-4 sm:p-5"><h3 className="text-sm font-medium">在 agent 本机完成配对</h3><p className="mt-2 text-sm leading-7 text-muted-foreground">在部署包目录执行下面的绑定命令，或交给你本机的 agent 执行。它会自动注册本机 agent 到 platform、保存网页授权，并配置连接。保持 agent 与 helper 运行。</p><p className="mt-2 text-xs leading-6 text-muted-foreground">本次网页授权有效期 30 天。命令执行后重启本机 Gateway，使网页连接生效。源码目录运行时，将脚本路径改为 tools/release/early_access/configure_hermes.py。</p>{w.identity.virtualUrn && <div className="mt-3 rounded-xl border bg-background p-3"><div className="flex justify-end"><CopyValue value={pairCommand} label="复制绑定命令" /></div><code className="block select-all break-all text-xs leading-6">{pairCommand}</code></div>}<div className="mt-4 flex flex-wrap gap-2"><Button className="gap-2 rounded-xl" disabled={w.identityBusy || !w.identity?.virtualUrn || !!w.busy.capabilities} onClick={() => void w.invoke("capabilities")}>{w.busy.capabilities ? <Loader2 className="h-4 w-4 animate-spin" /> : <ArrowRight className="h-4 w-4" />}{w.busy.capabilities ? "检查中…" : w.capabilitySnapshot ? "重新检查连接" : "立即检查连接"}</Button>{w.identity?.virtualUrn && <Button variant="ghost" size="sm" className="rounded-xl text-xs" disabled={w.identityBusy || !!w.busy.capabilities} onClick={w.createIdentity}>{w.identityBusy ? "注册中…" : "重新注册身份"}</Button>}</div></div>
      </div>
