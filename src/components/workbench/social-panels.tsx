...
export function FriendRequests({ workbench: w }: { workbench: Workbench }) {
  ...
  return <section className="mx-5 mb-5 rounded-2xl border p-4" aria-label="好友请求"><h3 className="text-sm font-medium">好友请求</h3><div className="mt-3 space-y-3">{requests.slice().reverse().map(request => {
    ...
    return <article id={`subject-${id}`} key={id} className="rounded-xl bg-muted/40 p-3"><div className="flex flex-wrap justify-between gap-2 text-xs"><span>{incoming ? "收到的请求" : "发出的请求"} · {pending ? incoming ? "等待你处理" : "等待对方接受" : request.status === "accepted" ? "已建立连接" : "已拒绝"}</span><time className="text-muted-foreground">{displayTime(request.updated_at || request.created_at)}</time></div><p className="mt-2 break-all font-mono text-xs leading-6">{string(request.peer_urn)}</p>{incoming && pending && <div className="mt-3 flex gap-2"><Button size="sm" disabled={!w.mutations.ready || !w.mutations.canMutate("contacts.respond") || !!locked} onClick={() => void w.mutations.run("contacts.respond", { request_id: id, decision: "accept" })}>接受好友请求</Button><Button size="sm" variant="outline" disabled={!w.mutations.ready || !w.mutations.canMutate("contacts.respond") || !!locked} onClick={() => void w.mutations.run("contacts.respond", { request_id: id, decision: "reject" })}>拒绝</Button></div>}<SocialFeedback workbench={w} method="contacts.respond" subject={id} /></article>;
  })}</div></section>;
}
