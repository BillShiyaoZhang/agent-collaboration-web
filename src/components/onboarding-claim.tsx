...
function ApprovalButton({ busy, approve }: { busy: boolean; approve: () => void }) {
  ...
  return <Button className="w-full rounded-xl" onClick={approve} disabled={busy || !allowed}>
    {busy && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{allowed ? "授权并连接这个 Hermes" : "请先确认或恢复政策使用"}
  </Button>;
}
