...
function AgentWorkbench({ agent, initial }: { agent: Connection; initial: WorkspaceAgent }) {
  ...
  return <div className="mx-auto max-w-6xl space-y-6 pb-6">
    ...
    {!!tabs.length && <section className="overflow-hidden rounded-2xl border bg-card shadow-sm">
      ...
      {visibleTab === "conversation" ? <ConversationPanel workbench={w} agentName={agent.name} /> : visibleTab && snapshotMethod ? <div role="tabpanel" id={`panel-${visibleTab}`} aria-labelledby={`tab-${visibleTab}`}><div className="flex items-start justify-between gap-3 p-5"><div><h2 className="font-medium">{tabs.find(tab => tab.id === visibleTab)?.label}</h2><p className="mt-1.5 text-xs leading-5 text-muted-foreground">{descriptions[visibleTab]}</p>{snapshot && <p className="mt-2 text-[11px] text-muted-foreground">已保存 · 最近同步 {displayTime(snapshot.time / 1000)}</p>}</div><Button variant="outline" size="sm" className="gap-1.5 rounded-xl" disabled={!!w.busy[snapshotMethod]} onClick={() => void w.invoke(snapshotMethod)}><RefreshCw className={cn("h-3.5 w-3.5", w.busy[snapshotMethod] && "animate-spin")} />刷新</Button></div>{(w.busy[snapshotMethod] || w.errors[snapshotMethod]) && <div className="px-5 pb-4"><RequestFeedback busy={w.busy[snapshotMethod]} error={w.errors[snapshotMethod]} onRetry={() => void w.invoke(snapshotMethod)} /></div>}{!snapshot && !w.errors[snapshotMethod] && !["offline", "needs_pairing"].includes(w.sync.status) && <div aria-hidden className="space-y-3 px-5 pb-6">{[1, 2, 3].map(key => <div key={key} className="h-24 animate-pulse rounded-2xl bg-muted/60" />)}</div>}{!snapshot && ["offline", "needs_pairing"].includes(w.sync.status) && <p role="status" className="px-5 pb-8 text-sm leading-6 text-muted-foreground">这部分内容尚未同步。连接恢复后会自动读取。</p>}{snapshot && (visibleTab === "contacts" ? <ContactsSnapshot data={snapshot.data} /> : visibleTab === "tasks" ? <TasksSnapshot data={snapshot.data} /> : <InboxSnapshot data={snapshot.data} />)}</div> : null}
    </section>}
    <p className="flex items-start justify-center gap-1.5 px-2 text-center text-[11px] leading-5 text-muted-foreground"><ShieldCheck className="mt-0.5 h-3.5 w-3.5 shrink-0" />消息、联系人与会话记录保存在你的账号中，并自动从 agent 同步。</p>
  </div>;
}
