...
export function DashboardHeader({ user }: { user: { name?: string | null; email?: string | null } }) {
  ...
  return <header className="relative z-20 flex min-h-16 shrink-0 flex-wrap items-center gap-3 border-b bg-card/75 px-4 py-2 backdrop-blur-xl sm:px-8">
    ...
    <nav aria-label="面包屑" className="flex min-w-0 items-center gap-2 text-sm"><Link href="/dashboard/chats" className="inline-flex min-h-6 items-center whitespace-nowrap rounded-sm text-muted-foreground transition-colors hover:text-foreground" aria-current={pathname === "/dashboard/chats" ? "page" : undefined}>聊天</Link>{pathname !== "/dashboard/chats" && <><ChevronRight className="h-3.5 w-3.5 shrink-0 text-muted-foreground" /><span className="truncate font-medium" aria-current="page">{pathname === "/dashboard/notifications" ? "提醒中心" : pathname === "/dashboard/collaborations" ? "合作" : pathname === "/dashboard/contacts" ? "联系人" : pathname === "/dashboard/me" ? "我" : pathname === "/dashboard/agents" ? "我的连接" : "对话"}</span></>}</nav>
