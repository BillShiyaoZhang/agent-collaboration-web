...
const filterOptions = [["unread", "未读"], ["pending", "待我处理"], ["all", "全部"]] as const;
...
export default function NotificationsPage() {
  ...
  return <div className="mx-auto max-w-4xl space-y-6">
    ...
    <details className="rounded-2xl border bg-card" aria-label="提醒设置">
      <summary className="cursor-pointer px-4 py-3 text-sm font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring sm:px-5">提醒设置 <span className="ml-2 text-xs font-normal text-muted-foreground">{notifications.enabled ? notifications.background ? "后台推送已开启" : "仅页面打开时提醒" : "系统提醒已关闭"}</span></summary>
      <div className="space-y-4 border-t px-4 py-4 sm:px-5">
        <div className="flex flex-wrap items-center justify-between gap-4"><div className="min-w-0 flex-1"><h2 className="text-sm font-medium">此设备的系统提醒</h2><p className="mt-1 max-w-xl text-sm leading-6 text-muted-foreground">{notifications.background ? "已开启后台推送：关闭页面后也可提醒，需浏览器和系统允许后台通知。推送仅含概括，不含聊天或授权内容。" : "开启后，在页面失去焦点或切换应用时提醒你；支持的浏览器还可开启关闭页面后的后台推送。锁屏仅显示概括。"}</p>{notifications.permission === "denied" && <p className="mt-1 text-sm text-amber-800">浏览器已阻止系统提醒，可在浏览器网站设置中调整。</p>}{notifications.permission === "unsupported" && <p className="mt-1 text-sm text-muted-foreground">当前浏览器或连接不支持系统提醒。</p>}</div><Button variant="outline" disabled={["unsupported", "denied"].includes(notifications.permission)} onClick={() => void notifications.toggleSystem()} className="h-auto max-w-full gap-2 whitespace-normal py-2 text-center">{notifications.enabled ? <BellOff className="h-4 w-4 shrink-0" /> : <Bell className="h-4 w-4 shrink-0" />}{notifications.enabled ? "关闭系统提醒" : "开启系统提醒"}</Button></div>
        <div className="border-t pt-4"><div className="flex flex-wrap items-center gap-3"><Button variant="outline" className="h-auto max-w-full whitespace-normal py-2 text-center" disabled={!notifications.enabled} onClick={() => void notifications.testSystem()}>发送测试提醒</Button>{notifications.enabled && !notifications.background && notifications.pushAvailable && <Button variant="outline" className="h-auto max-w-full whitespace-normal py-2 text-center" onClick={() => void notifications.enableBackground()}>开启关闭页面后的提醒</Button>}</div>{notifications.diagnostic && <p role="status" className="mt-3 text-sm leading-6">{notifications.diagnostic}</p>}<details className="mt-3 text-sm text-muted-foreground"><summary className="inline-flex min-h-8 cursor-pointer items-center">提醒诊断</summary><p className="mt-2 leading-6">浏览器权限：{({ granted: "已允许", denied: "已阻止", default: "尚未允许", unsupported: "不支持" } as Record<string, string>)[notifications.permission] || notifications.permission} · 当前页面：{notifications.away ? "后台或未聚焦" : "前台聚焦"}。测试只检查提醒通道，不会发送协作消息或批准操作。浏览器确认显示请求也不代表用户已经看到横幅；若没有横幅，请检查系统勿扰与网站通知设置。</p></details></div>
      </div>
    </details>
    <div role="tablist" aria-label="提醒筛选" className="flex flex-wrap gap-2">{filterOptions.map(([value, label], index) => <button type="button" role="tab" id={`notifications-tab-${value}`} aria-controls="notifications-panel" aria-selected={filter === value} tabIndex={filter === value ? 0 : -1} key={value} onClick={() => setFilter(value)} onKeyDown={event => { let next = index; if (event.key === "ArrowRight" || event.key === "ArrowDown") next = (index + 1) % filterOptions.length; else if (event.key === "ArrowLeft" || event.key === "ArrowUp") next = (index + filterOptions.length - 1) % filterOptions.length; else if (event.key === "Home") next = 0; else if (event.key === "End") next = filterOptions.length - 1; else return; event.preventDefault(); const nextValue = filterOptions[next][0]; setFilter(nextValue); document.getElementById(`notifications-tab-${nextValue}`)?.focus(); }} className={`rounded-xl px-4 py-2 text-sm ${filter === value ? "bg-primary text-primary-foreground" : "border bg-card text-muted-foreground"}`}>{label}</button>)}</div>
    <section role="tabpanel" id="notifications-panel" aria-labelledby={`notifications-tab-${filter}`} tabIndex={0}>
      ...
      {items.some(item => item.requiresAction) && <p className="mb-3 text-xs leading-6 text-muted-foreground">待处理提醒请打开当前事项核对并决定；如网页尚未开放此项确认，可从 agent 的原生渠道回应。标记已读只更新提醒。</p>}
      {!items.length && <div className="rounded-2xl border border-dashed px-6 py-16 text-center"><Bell className="mx-auto mb-3 h-7 w-7 text-muted-foreground" /><p className="font-medium">{filter === "pending" ? "暂时没有待你处理的事项" : filter === "unread" ? "没有未读提醒" : "这里暂时没有提醒"}</p><p className="mt-2 text-sm text-muted-foreground">已授权连接的新消息和待确认事项会自动汇集到这里。</p></div>}
