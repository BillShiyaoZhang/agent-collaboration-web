...
export default function NotificationsPage() {
  ...
  return <div className="mx-auto max-w-4xl space-y-6">
    ...
    <section className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border bg-card p-3 sm:p-5"><div className="min-w-0"><h2 className="text-sm font-medium">此设备的系统提醒</h2><p className="mt-1 max-w-xl text-sm leading-6 text-muted-foreground">{notifications.background ? "已开启后台推送：关闭页面后也可提醒，需浏览器和系统允许后台通知。推送仅含概括，不含聊天或授权内容。" : "开启后，在页面失去焦点或切换应用时提醒你；支持的浏览器还可开启关闭页面后的后台推送。锁屏仅显示概括。"}</p>{notifications.permission === "denied" && <p className="mt-1 text-sm text-amber-800">浏览器已阻止系统提醒，可在浏览器网站设置中调整。</p>}{notifications.permission === "unsupported" && <p className="mt-1 text-sm text-muted-foreground">当前浏览器或连接不支持系统提醒。</p>}</div><Button variant="outline" disabled={["unsupported", "denied"].includes(notifications.permission)} onClick={() => void notifications.toggleSystem()} className="h-auto max-w-full gap-2 whitespace-normal py-2 text-center">{notifications.enabled ? <BellOff className="h-4 w-4 shrink-0" /> : <Bell className="h-4 w-4 shrink-0" />}{notifications.enabled ? "关闭系统提醒" : "开启系统提醒"}</Button></section>
    <section className="rounded-2xl border bg-card p-3 sm:p-5"><div className="flex flex-wrap items-center gap-3"><Button variant="outline" className="h-auto max-w-full whitespace-normal py-2 text-center" disabled={!notifications.enabled} onClick={() => void notifications.testSystem()}>发送测试提醒</Button>{notifications.enabled && !notifications.background && notifications.pushAvailable && <Button variant="outline" className="h-auto max-w-full whitespace-normal py-2 text-center" onClick={() => void notifications.enableBackground()}>开启关闭页面后的提醒</Button>}<span className="text-sm text-muted-foreground">{notifications.enabled ? notifications.background ? "后台推送已开启" : "仅页面打开时提醒" : "系统提醒已关闭"}</span></div>{notifications.diagnostic && <p role="status" className="mt-3 text-sm leading-6">{notifications.diagnostic}</p>}<details className="mt-3 text-sm text-muted-foreground"><summary className="cursor-pointer">提醒诊断</summary><p className="mt-2 leading-6">浏览器权限：{({ granted: "已允许", denied: "已阻止", default: "尚未允许", unsupported: "不支持" } as Record<string, string>)[notifications.permission] || notifications.permission} · 当前页面：{notifications.away ? "后台或未聚焦" : "前台聚焦"}。测试只检查提醒通道，不会发送协作消息或批准操作。浏览器确认显示请求也不代表用户已经看到横幅；若没有横幅，请检查系统勿扰与网站通知设置。</p></details></section>
