...
export default function NotificationsPage() {
  ...
  return <div className="mx-auto max-w-4xl space-y-6">
    ...
    <section className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border bg-card p-5"><div><h2 className="text-sm font-medium">此设备的系统提醒</h2><p className="mt-1 max-w-xl text-sm leading-6 text-muted-foreground">{notifications.background ? "已开启后台推送：关闭页面后也可提醒，需浏览器和系统允许后台通知。推送仅含概括，不含聊天或授权内容。" : "开启后，在页面失去焦点或切换应用时提醒你；支持的浏览器还可开启关闭页面后的后台推送。锁屏仅显示概括。"}</p>{notifications.permission === "denied" && <p className="mt-1 text-sm text-amber-800">浏览器已阻止系统提醒，可在浏览器网站设置中调整。</p>}{notifications.permission === "unsupported" && <p className="mt-1 text-sm text-muted-foreground">当前浏览器或连接不支持系统提醒。</p>}</div><Button variant="outline" disabled={["unsupported", "denied"].includes(notifications.permission)} onClick={() => void notifications.toggleSystem()} className="gap-2">{notifications.enabled ? <BellOff className="h-4 w-4" /> : <Bell className="h-4 w-4" />}{notifications.enabled ? "关闭系统提醒" : "开启系统提醒"}</Button></section>
    ...
    <section role="tabpanel" id="notifications-panel" aria-labelledby={`notifications-tab-${filter}`} tabIndex={0}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><span className="text-xs text-muted-foreground">{item.agentName}</span>{item.unread && <span className="rounded-full bg-primary/10 px-2 py-0.5 text-xs leading-5 text-primary">未读</span>}</div><span className={`text-xs ${item.requiresAction ? "text-amber-800" : "text-muted-foreground"}`}>{item.requiresAction ? "待你处理" : statusLabels[item.state]}</span></div>
