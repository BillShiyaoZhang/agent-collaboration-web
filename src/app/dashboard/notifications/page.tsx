...
export default function NotificationsPage() {
  ...
  return <div className="mx-auto max-w-4xl space-y-6">
    ...
    <section className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border bg-card p-5"><div><h2 className="text-sm font-medium">此设备的系统提醒</h2><p className="mt-1 max-w-xl text-xs leading-6 text-muted-foreground">开启后，在此页面保持打开、切到其他应用时提醒你。锁屏通知仅显示概括；关闭页面后请使用站内提醒中心查看。</p>{notifications.permission === "denied" && <p className="mt-1 text-xs text-amber-800">浏览器已阻止系统提醒，可在浏览器网站设置中调整。</p>}{notifications.permission === "unsupported" && <p className="mt-1 text-xs text-muted-foreground">当前浏览器或连接不支持系统提醒。</p>}</div><Button variant="outline" disabled={["unsupported", "denied"].includes(notifications.permission)} onClick={() => void notifications.toggleSystem()} className="gap-2">{notifications.enabled ? <BellOff className="h-4 w-4" /> : <Bell className="h-4 w-4" />}{notifications.enabled ? "关闭系统提醒" : "开启系统提醒"}</Button></section>
