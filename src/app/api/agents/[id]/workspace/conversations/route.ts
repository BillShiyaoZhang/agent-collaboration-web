...
const bodySchema = z.object({ conversationId: stableId.nullable(), title: z.string().trim().min(1).max(120).optional(), archived: z.boolean().optional(), deleted: z.boolean().optional(),
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = z.object({ q: z.string().max(200).optional(), archived: z.enum(["active", "archived", "all"]).optional(), deleted: z.enum(["active", "deleted", "all"]).optional(), before: stableId.optional(), limit: z.coerce.number().int().min(1).max(50).optional() })
