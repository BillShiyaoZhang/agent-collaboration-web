...
async function main(){
 ...
 checks.push('标记已读减少未读数，不减少仍待确认数');
 ...
 await page.bringToFront();await page.getByRole('button',{name:'开启系统提醒',exact:true}).click();await page.getByRole('button',{name:'关闭系统提醒',exact:true}).waitFor();
 ...
 checks.push('设备开关由用户点击触发；浏览器通知已替换为测试桩，未发送真实系统通知');
