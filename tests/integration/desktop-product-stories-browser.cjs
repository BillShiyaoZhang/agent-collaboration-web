...
function conversationRow(page, id) { return page.locator('[data-conversation-id=' + JSON.stringify(id) + ']'); }
async function selectConversation(page, id) {
  await page.getByRole('button', { name: '对话', exact: true }).click();
  await page.getByLabel('搜索已保存对话', { exact: true }).fill('');
...
async function navigate(page, label) {
  ...
  await page.getByRole('navigation', { name: mobile ? '手机主导航' : '主导航', exact: true }).getByRole('link', { name: label, exact: true }).click();
}
...
async function relatedDetails(page) {
  const summary = page.locator('summary').filter({ hasText: /^关联合作 ·/ });
...
async function main() {
  ...
  await page.getByLabel('对话标题', { exact: true }).fill('产品讨论主题');
  ...
  await page.getByText('确切委托范围：', { exact: false }).waitFor();
  ...
  const contact = page.locator('#subject-product-peer');
  ...
  assert.equal(persistedTask.recordStates.some(value => value.kind === 'collaboration' && value.id === prepare.params.task_id && value.deleted), true);
  await navigate(page, '合作');
  assert.equal(await page.locator('a[href*="subject=' + prepare.params.task_id + '"]').count(), 0, 'aggregate list hides the same task across collaboration aliases');
  ...
  await page.locator('a[href*="subject=' + prepare.params.task_id + '"]').waitFor();
  await collaborations(page);
  await taskCard.getByText('双方已取消约定', { exact: true }).waitFor();
...
main().catch(async error => {
  ...
  if (activePage) { try { await activePage.screenshot({ path: path.join(out, 'failure.png'), fullPage: true }); fs.writeFileSync(path.join(out, 'failure.json'), JSON.stringify({ error: error.message, body: await activePage.locator('body').innerText(), writes, metadataWrites, errors, recordWrites }, null, 2)); } catch {} }
  process.exitCode = 1;
